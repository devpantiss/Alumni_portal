...
test('false filters select offline, inactive and unverified alumni',()=>{
  for (const key of ['online','active','verified']) {
    const expected=alumni.filter(person=>person[key]===false);
    assert.ok(expected.length>0);
    assert.deepEqual(filterAlumni(alumni,{[key]:false}),expected);
    assert.deepEqual(filterAlumni(alumni,{[key]:'false'}),expected);
  }
});
